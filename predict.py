...
def parse_args():
    ...
    parser.add_argument('--model', type=str, required=True, help='Base model architecture',
                        choices=[config.MODEL_RESNET50, config.MODEL_RESNET152, config.MODEL_INCEPTION_V3,
                                 config.MODEL_VGG16, config.MODEL_XCEPTION])
...
def predict(path):
    ...
    if args.store_activations:
        predictions = np.argsort(-out, axis=1)[:,:3] # top3 scores
        for i, p in enumerate(predictions):
            recognized_class1 = list(classes_in_keras_format.keys())[list(classes_in_keras_format.values()).index(p[0])]
            recognized_class2 = list(classes_in_keras_format.keys())[list(classes_in_keras_format.values()).index(p[1])]
            #recognized_class3 = list(classes_in_keras_format.keys())[list(classes_in_keras_format.values()).index(p[2])]
            score1 = str(out[i][p[0]])
            score2 = str(out[i][p[1]])
            #score3 = str(out[i][p[2]])
            recognized_class = recognized_class1+":"+score1+","+recognized_class2+":"+score2+","
...
if __name__ == '__main__':
    ...
    # predictions[:] = np.argsort(-out, axis=1)[:,:3]  #np.argmax(out, axis=1)                                                                         │······················································
    # for i, p in enumerate(predictions):                                                                                                              │······················································
    #     recognized_class = list(classes_cows.keys()[list(classes_cows.values()).index(p)]                                                            │······················································
    #     print(p, recognized_class)                                                                                                                   │······················································
    #     all_coordinates[i][0] = recognized_class