...
if args.output_model_file == '':
    ...
    args.output_model_pbtxt = str(Path(args.input_model_file).name) + '.pbtxt'
else:
    args.output_model_pbtxt = args.output_model_file + 'txt'
...
try:
    net_model = load_model(weight_file_path,custom_objects={'center_loss_fn': center_loss_fn})
...
graph_io.write_graph(constant_graph, output_fld, args.output_model_pbtxt, as_text=True)
